...
Call log:
  - waiting for locator('//iframe[@id=\'rightMenu\']').contentFrame().locator('//input[@id=\'btnEditPers\']')
          ...
          - row "Employee Id Employee Name Job Title Employment status Sub-Division Supervisor" [ref=f1e25]:
            ...
            - cell "Supervisor" [ref=f1e38]:
              - link "Supervisor" [ref=f1e39] [cursor=pointer]:
                - /url: "#"
        - rowgroup [ref=f1e40]:
          - row "9450 Hibban Shaik IT - - - -" [ref=f1e41]:
            - cell [ref=f1e42]:
              - checkbox [ref=f1e43]
            - cell "9450" [ref=f1e44]
            - cell "Hibban Shaik IT" [ref=f1e45]:
              - link "Hibban Shaik IT" [ref=f1e46] [cursor=pointer]:
                - /url: ./CentralController.php?menu_no_top=hr&id=9450&capturemode=updatemode&reqcode=EMP&currentPage=1
            - cell "-" [ref=f1e47]
            - cell "-" [ref=f1e48]
            - cell "-" [ref=f1e49]
            - cell "-" [ref=f1e50]
...
```ts
  ...
  35 |     }
  36 |     async Empedit () {
  37 |         const Frame = this.page.frameLocator(this.frame_rightmenu);
  38 |         await Frame.locator(this.dropdownvalue).selectOption({ value: "0" });
  39 |         await Frame.locator(this.textbox_number).fill(this.empnumber);
  40 |         await Frame.locator(this.search).click();
  41 |         await this.page.waitForTimeout(3000);
  42 |         console.log("Checkbox count:",
  43 |         await Frame.locator(this.checkbox).count());
  44 |         //await Frame.locator(this.checkbox).waitFor({ state: "visible" });
  45 |         await Frame.locator(this.emplink).count();
  46 |         //await Frame.locator(this.emplink).click();
> 47 |         await Frame.locator(this.edit_button).click();
     |                                               ^ Error: locator.click: Test timeout of 30000ms exceeded.
  48 |         await Frame.locator(this.dirthbd).fill(this.dob);
  49 |         await Frame.locator(this.save).click();
  50 |         console.log('Edit edited successfully');
  51 | 
  52 | 
  53 |     }
  54 | }
